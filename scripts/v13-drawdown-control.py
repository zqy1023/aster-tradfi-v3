...
def run_roll(year, dd_limit=0.30):
    ...
    for i in range(20, len(c)):
        ...
        if not bull:
            ...
            continue
        # 回撤控制: 当前回撤>limit → 减半仓
        cur_dd = equity / peak - 1
        scale = 0.5 if cur_dd < -dd_limit else 1.0
        if i - last_pick >= 5:
            mom = {}
            for s in SYMS:
                cc = c[s].dropna()
...
print('=== v13: v12 + 回撤控制 ===')
for name, limit in [('v12无控制', 0.99), ('v13回撤30%降半仓', 0.30), ('v13回撤20%降半仓', 0.20)]:
    eqs, dds = [], []
    line = f'{name:>18} |'
    for year in [2022, 2023, 2024, 2025, 2026]:
        eq, dd = run_roll(year, limit)
        if eq is None: continue
        eqs.append(eq); dds.append(dd)
    print(f'{line} 均值{eqs and np.mean(eqs):>9,.0f} | 回撤{np.mean(dds)*100:5.1f}%')
